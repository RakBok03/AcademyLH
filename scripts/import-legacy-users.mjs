...
async function callPuzzleBot(method, params = {}) {
  ...
  const maxAttempts = Number(process.env.PUZZLEBOT_IMPORT_RETRIES || 4);
  const timeoutMs = Number(process.env.PUZZLEBOT_IMPORT_TIMEOUT_MS || 15000);
  let lastError;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      await sleep(Math.max(500, puzzleDelayMs) * attempt);
    }
  }
  throw lastError;
}
...
async function importUser(client, user, vars, quizMap, sectionMap) {
  ...
  await client.query("DELETE FROM quiz_attempts WHERE user_id = $1 AND answers->>'legacyImport' = 'true'", [dbUser.id]);
  await client.query("DELETE FROM point_events WHERE user_id = $1 AND source_type IN ('legacy_task', 'legacy_adjustment')", [dbUser.id]);
